...
class Player {
     ...
     * @return array array of objects, one object mean one player
     */
    public static function getAllPlayersByTeam($connection, $player_club_id, $columns = "*"){
        $sql = "SELECT $columns
                FROM player_user
                WHERE player_Id != 0 and player_club_id = :player_club_id
                ORDER BY second_name";
        ...
        try {
            if($stmt->execute()){
                ...
                throw new Exception ("Príkaz pre získanie všetkých dát o hráčoch podľa teamu sa nepodaril");
            }
        } catch (Exception $e){
            // 3 je že vyberiem vlastnú cestu k súboru
            error_log("Chyba pri funckii getAllPlayersByTeam, príkaz pre získanie informácií z databázy zlyhal\n", 3, "./errors/error.log");
            echo "Výsledná chyba je: " . $e->getMessage();
        }
    }
